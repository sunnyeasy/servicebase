...
public class ClusterZookeeperRegistry {
    ...
    public ClusterZookeeperRegistry(ZkClient zkClient, PushNode pushNode, String clusterPath) {
    ...
    private void createPushNode() {
        String nodePath = clusterPath + PATH_CONNECTOR + pushNode.getHostname();
        if (zkClient.exists(nodePath)) {
            zkClient.delete(nodePath);
        }

        zkClient.createEphemeral(nodePath, JSON.toJSONString(pushNode));
    }
    ...
    public Map<String, PushNode> getPushNodeMap() {
        return pushNodeMap;
    }
